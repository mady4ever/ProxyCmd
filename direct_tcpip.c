...
const char *username = "mahepate";
const char *password = "Nbv12345";

/*
	Following are some of the default parameters. 
*/
...
int LocalPortForwading(char *username, char *password, char *server_ip, char *local_listenip, unsigned int local_listenport, char *remote_desthost, unsigned int remote_destport);
enum {
    AUTH_NONE = 0,
    AUTH_PASSWORD,
    AUTH_PUBLICKEY
...
void log_print(char* filename, int line, char *fmt,...)
{
    ...
    if(SESSION_TRACKER > 0)
      fp = fopen ("LocalTunnel.txt","a+");
    else
      fp = fopen ("LocalTunnel.txt","w");
...
int main(int argc, char *argv[])
{
		...
		LocalPortForwading( username, password, server_ip, local_listenip, local_listenport,remote_desthost,remote_destport);		
		LOG_PRINT("Exiting the function");	
}
int LocalPortForwading( char *username, char *password, char *server_ip, char *local_listenip, unsigned int local_listenport, char *remote_desthost, unsigned int remote_destport)
{
...
//I am adding here a while loop for contineously creating the new socs and listen it.
